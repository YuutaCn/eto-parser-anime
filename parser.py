...
for card_url in get_url():
  ...
  def frame_find(el):
    if el == None:
      return 'Сожалеем, но аниме пока что не добавлено.'
    return el.get('src')
  ...
  anime_status = text_find(anime_sheets.find('a'))
  ...
  anime_season = text_find(anime_sheets.select_one('.pmovie__genres:nth-child(3) > .anime-r > a'))
  anime_type = text_find(anime_sheets.select_one('.pmovie__genres:nth-child(4) > .anime-r > a'))
  ...
  anime_studio = text_find(anime_sheets.select_one('.pmovie__genres:nth-child(6) > .anime-r'))
  anime_cover = 'https://yummyanime.org' + soup.select_one('.pmovie__poster > img').get('data-src')
  anime_desc = text_find(soup.find('div', class_='page__text full-text clearfix'))
